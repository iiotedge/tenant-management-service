...
RUN --mount=type=cache,target=/root/.m2 \
    --mount=type=secret,id=gpr,target=/tmp/settings.xml \
    mvn -s /tmp/settings.xml -B -U -DskipTests dependency:go-offline

# Build sources
COPY src ./src
RUN --mount=type=cache,target=/root/.m2 \
    --mount=type=secret,id=gpr,target=/tmp/settings.xml \
    mvn -s /tmp/settings.xml -B -DskipTests=${SKIP_TESTS} clean package
...
LABEL org.opencontainers.image.title="tenant-management-service" \
      ...
      org.opencontainers.image.source="https://github.com/iotmining/platform-build" \
